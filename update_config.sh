...
config_path="/data_judian/config/user_site/"
if [  -d $config_path ];then
    echo "----------------------------------------git pull --rebase user_site 输出----------------------------------------'$enter'"
    cd $config_path
    echo -e "cd /data_judian/config/user_site/'$enter'"
...
config_path="/data_judian/config/server/"
if [  -d $config_path ];then
    echo "----------------------------------------git pull --rebase  server 输出----------------------------------------'$enter'"
    cd $config_path
    echo -e "cd /data_judian/config/server/'$enter'"
    pull_ret=\`git pull --rebase origin master 2>&1\`
    echo -e "$pull_ret'$enter'"
    flag=\`echo $pull_ret | awk "/'$exception_str'/"\`
    if [ "$flag" = "" ];then
